...
const fetchUsers = async () => {
  try {
    const res = await fetch('https://specificationmvp.onrender.com/api/admin');
...
const handleSave = async (user) => {
  try {
      const response = await fetch(`https://specificationmvp.onrender.com/api/admin/${user._id}`, {
          method: "PUT",
  ...
  const handleDelete = async (id) => {
    try {
      const res = await fetch(`https://specificationmvp.onrender.com/api/admin/${id}`, { method: "DELETE" });
      const data = await res.json();
      if (data.success) {
